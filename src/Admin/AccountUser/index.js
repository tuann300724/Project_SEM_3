...
function AccountUser() {
  ...
  const fetchUser = async () => {
    try {
      ...
      console.log("data user", result.data.data);
    } catch (error) {
  ...
  const handleStatusToggle = async (id, isActive) => {
    try {
      ...
      await axios.put(`http://localhost:5223/changeActive/${id}`, {
            ...
            return (
              <div className="account-card" key={account.id}>
                ...
                <div className="account-actions">
                  <button
                    className={`btn btn-${
                      account.isActive ? "success" : "secondary"
                    } btn-sm mt-2 m-2 pl-3 pr-3 pt-2 pb-2`}
                    onClick={() =>
                      handleStatusToggle(account.id, account.isActive)
                    }
                  >
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigate(`/admin/AccountDetail/${account.id}`)
                    }
                  >
                    View
                  </button>
